...
library(tidyverse)
library(here)
library(janitor) # good for data cleaning types of things 
library(skimr)
```
...
toolik_weather <- read_csv(here::here("data", "data-raw", "toolik_weather.csv"), 
                           na = "#N/A") |> # replace #N/A calues with NA
  clean_names() # makes all column headers lowercase
  
  # here::here -> get 'here' function from 'here' package

toolik_mean_temp_month <- ggplot(data = toolik_weather,
       aes(x = month, 
           y = daily_air_temp_mean_c,
           color = daily_air_temp_mean_c)) +
  geom_jitter() +
  theme_classic() +
...
# save a ggplot object to a selected folder 
ggsave(plot = toolik_mean_temp_month,
       here::here("figs", "toolik_mean_temp_month.png"),
       height = 6,
       width = 7)
```
